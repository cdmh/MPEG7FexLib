...
double const dcmin=0.0, const dcmax=255.0;	// 2001.01.31 - yjyu@samsung.com
double const stdmin=1.309462, const stdmax=109.476530;

double const mmax[5][6]={{18.392888,18.014313,18.002143,18.083845,18.046575,17.962099},
...
double const mmin[5][6]={{6.549734,8.886816,8.885367,6.155831,8.810013,8.888925},
...
double const dmax[5][6]={{21.099482,20.749788,20.786944,20.847705,20.772294,20.747129},
...
double const dmin[5][6]={{9.052970,11.754891,11.781252,8.649997,11.674788,11.738701},
...
// CH 18/5/16 thread safety by keeping data inside a class instance
...
void CHomogeneousTexture_Extractor::FeatureExtraction(unsigned char* image)
{
    ...
    impl.vatomdesign();
    impl.hatomdesign();
    impl.module(image, image_height, image_width);

    m_dc = (int)impl.dc;
    m_std = (int)impl.stdev;

    for(int n=0;n<5;n++)
        for(int m=0;m<6;m++)
            mean2[n][m]=(int)(impl.vec[n][m]);		// mean : 30 energy features
    for(int n=0;n<5;n++)
        for(int m=0;m<6;m++)
            dev2[n][m]=(int)(impl.dvec[n][m]);		// dev	: 30 deviation features
}
